...
//     });
    ...
    //         expect(res).to.have.status(200);
    //         expect(res.body).to.have.property('message').equal(`Get contact by ID ${contact._id}`);
    ...
    //         const res = await server.(http://localhost:5001/api/contact).get(`/${contact._id}`).send(updatedData);
    //         expect(res).to.have.status(200);
    ...
    //         const contact = await Contact.findOne(); // Find an existing contact from the database
    //         const res = await server.(http://localhost:5001/api/contact).get(`/${contact._id}`);
    //         expect(res).to.have.status(200);
